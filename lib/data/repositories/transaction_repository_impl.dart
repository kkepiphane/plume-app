...
class TransactionRepository {
  ...
  FinancialSummaryEntity computeSummary(List<TransactionEntity> txs, double budget) {
    ...
    final budgetUsage = budget > 0 ? (totalExpenses / budget) * 100 : 0.0;

    // Daily balances for chart (last 30 days)
    ...
    for (final tx in txs) {
      ...
      if (existing == null) {
        ...
        dailyMap[key] = DailyBalance(
          ...
          balance: existing.balance + (tx.isIncome ? tx.amount : -tx.amount),
        );
      }
    }
