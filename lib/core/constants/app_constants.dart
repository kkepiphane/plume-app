...
// ─────────────────────────────────────────────────────────────────────────────
// EXPENSE CATEGORIES
// Rule: 'icon' key MUST match a case in CategoryIcons.get()
// ─────────────────────────────────────────────────────────────────────────────
class ExpenseCategories {
  ExpenseCategories._();

  static const List<Map<String, dynamic>> defaults = [
    {'id': 'transport',       'label': 'Transport',       'icon': 'transport',    'color': 0xFF4CAF50},
    {'id': 'nourriture',      'label': 'Nourriture',      'icon': 'food',         'color': 0xFFFF9800},
    {'id': 'connexion',       'label': 'Connexion',       'icon': 'wifi',         'color': 0xFF2196F3},
    {'id': 'loyer',           'label': 'Loyer',           'icon': 'home',         'color': 0xFF9C27B0},
    {'id': 'sante',           'label': 'Santé',           'icon': 'health',       'color': 0xFFF44336},
    {'id': 'loisirs',         'label': 'Loisirs',         'icon': 'leisure',      'color': 0xFFE91E63},
    {'id': 'vetements',       'label': 'Vêtements',       'icon': 'clothes',      'color': 0xFF00BCD4},
    {'id': 'education',       'label': 'Éducation',       'icon': 'education',    'color': 0xFF3F51B5},
    {'id': 'eau_electricite', 'label': 'Eau/Électricité', 'icon': 'utilities',    'color': 0xFFFF5722},
    {'id': 'famille',         'label': 'Famille',         'icon': 'family',       'color': 0xFF795548},
    {'id': 'autres_dep',      'label': 'Autres',          'icon': 'other',        'color': 0xFF607D8B},
  ];
}
...
class IncomeCategories {
  ...
  static const List<Map<String, dynamic>> defaults = [
    {'id': 'salaire',         'label': 'Salaire',         'icon': 'salary',       'color': 0xFF4CAF50},
    {'id': 'transfert',       'label': 'Transfert reçu',  'icon': 'transfer',     'color': 0xFF2196F3},
    {'id': 'business',        'label': 'Business',        'icon': 'business',     'color': 0xFFFF9800},
    {'id': 'investissement',  'label': 'Investissement',  'icon': 'investment',   'color': 0xFF9C27B0},
    {'id': 'freelance',       'label': 'Freelance',       'icon': 'freelance',    'color': 0xFF00BCD4},
    {'id': 'cadeau',          'label': 'Cadeau',          'icon': 'gift',         'color': 0xFFE91E63},
    {'id': 'autres_rev',      'label': 'Autres',          'icon': 'other',        'color': 0xFF607D8B},
  ];
}
...
class CategoryIcons {
  ...
  /// Maps the string icon key stored in the database to a Material IconData.
  static IconData get(String key) {
    switch (key.trim()) {         
      ...
      case 'family':      return Icons.people_rounded;
      ...
      // Legacy keys that may exist in old Hive data
      case 'network_wifi':      return Icons.wifi_rounded;
      case 'school_outlined':   return Icons.school_rounded;
      case 'famille':           return Icons.people_rounded;
      case 'other':
      default:            return Icons.category_rounded;
    }
  }

  /// Returns the category color from its stored int value.
  static Color color(int colorValue) => Color(colorValue);
}